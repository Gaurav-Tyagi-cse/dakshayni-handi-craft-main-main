...
function editAddress() {
    const newAddressName = prompt("Enter name:", document.getElementById("userAddressName").textContent);
    const newAddress = prompt("Enter your address:", document.getElementById("userAddress").textContent);

    if (newAddressName) document.getElementById("userAddressName").textContent = newAddressName;
    if (newAddress) document.getElementById("userAddress").textContent = newAddress;
}

function updateTotal() {
    const pricePerItem = 1199; // Fixed price per item
    const quantity = parseInt(document.getElementById("itemQuantity").value);
    const platformFee = 3;
    const totalPrice = pricePerItem * quantity + platformFee;

    document.getElementById("priceSummary").textContent = `₹${pricePerItem * quantity}`;
